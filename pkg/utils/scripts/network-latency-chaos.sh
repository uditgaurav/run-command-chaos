...
fault="delay ${NETWORK_LATENCY}ms ${JITTER}ms"

chaosCommand="sudo tc qdisc replace dev ${NETWORK_INTERFACE} root netem ${fault}"

if [ ! -z "$DESTINATION_IP" ]; then
    delayCommand="sudo tc qdisc replace dev ${NETWORK_INTERFACE} root handle 1: prio && sudo tc qdisc replace dev ${NETWORK_INTERFACE} parent 1:3 netem ${fault}"
